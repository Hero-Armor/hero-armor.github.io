# Hero Armor — хаб (головний репо)

Загальні правила проєкту — `/root/hero-armor/CLAUDE.md`, читати ПЕРШИМ. Тут — правила
самого хаба. Локальний шлях: `/root/hero-armor/hub` (переїзд 08.08.2026, було `ha_site_tmp`).
...
**Місток «зібране → data»:** `/root/hero-armor/tools/hero_armor_promote.py scan|list|accept` —
...
**Картинки: «щоб було видно, про що мова» (правило Івана 01.08).** Три механізми,
не плутай:
1. **Схеми, які ми малюємо самі** — генератор `<система>/model/*.py` (тільки stdlib,
   SVG пишеться текстом) → `.svg` поруч. На сторінку потрапляє ТЕКСТОМ, не картинкою:
   або через плейсхолдер у своєму шаблоні, або реєстрацією в `data/systems.json`
   полем `diagrams: [["lights/model/podium_plan.svg", "підпис"]]` — тоді generic-сторінка
   системи покаже їх сама. Текстом — бо тоді підписи перекладаються в англійську версію.
2. **Креслення від конструктора** — файл у `site/assets/` + запис у `figures` картки
   системи. Це растр, підписи всередині не перекладаються.
3. **Фото товарів** — `python3 site/bom_media.py` тягне справжнє фото зі сторінки
   товару (через свій Firecrawl на localhost:3002, Amazon віддає капчу звичайному
   urllib), звіряє ASIN, робить мініатюру в `site/assets/bom/` і дописує `img` у
   `data/bom.json`. Далі build сам ставить мініатюру в таблиці закупівлі і збирає
   смужку «Що це фізично» на сторінках типів світла (список позицій — `buy_match`
   у `lights/data/circuits.json`). Фото і `bom.json` комітяться; у CI нічого не тягнеться.
   Товари поза Amazon (Victron, AC Infinity) той самий скрипт бере через `og:image`
   сторінки — фото має бути й у них.

**Правило Івана 01.08.2026: рядок закупівлі = лінк + фото.** Кожна позиція
`bom.json`, яку можна купити, несе `url` на КОНКРЕТНУ сторінку товару (не на пошук
і не на магазин) і `img`. Ще не обрали — пишемо це словами в `note` («обираємо»,
«робимо самі», «за місцем»), порожню клітинку не лишаємо: вона читається як
«забув». Розхідники (кабелі, гермовводи, запобіжники) — не виняток. Перед
публікацією: `python3 site/bom_verify.py` відкриває кожен лінк і перевіряє, що
сторінка жива і про той самий ASIN — мертвий лістинг гірший за порожню клітинку,
бо виглядає перевіреним; `guard.py` рахує, скільки позицій ще без лінка чи фото.
...
## Закупівля видима на КОЖНІЙ сторінці (правило Івана, 01.08 і повторно 02.08)

Іван цими сторінками **закуповується**, а не лише читає їх. Тому будь-яка сторінка,
де згадана річ, яку можна купити, зобовʼязана показувати блок закупівлі:
смужка справжніх **фото** товарів + таблиця з **посиланням на картку конкретного
товару** (не на пошук, не на категорію), кількістю і ціною. Витратники — кабель,
гермовводи, запобіжники, кріплення — не виняток. Товар ще не обрано → так і
пишемо словами в рядку («ще не обрано, кандидати такі-то»); порожня клітинка
заборонена.

Як це підключається (руками нічого не переписувати — реєстр закупівлі один,
`data/bom.json`):

1. у даних підсистеми завести `buy: {match: [...слова...], note: "..."}`
   (приклади: `lights/data/back_core.json`, `lights/data/circuits.json`);
2. у `build.py` зібрати рядки з BOM за цими словами і віддати їх у
   `photo_wall_html(rows)` + `buy_table_html(rows, note)`;
3. фото тягне `python3 site/bom_media.py --only "<слово>"` — воно звіряє ASIN,
   кладе мініатюру в `site/assets/bom/` і дописує `img` у BOM.

**Сторінка нової підсистеми чи лабораторії без цього блоку не вважається
готовою.** Саме на цьому спіймали сторінку «Ядро на спині»: схеми, план і
креслення були, а купити по ній було нічого — Іван помітив 02.08.
