...
def build():
    ...
    gen_html = (
        ...
        f'заливати {_rlo:.1f}–{_rhi:.1f} разів на день. На одному баку на холостому ходу — '
        f'{gg["direct"]["hours_per_tank"][1]:.0f}–{gg["direct"]["hours_per_tank"][0]:.0f} год '
        f'(центр {gg["direct"]["hours_per_tank_mid"]:.0f} год при '
        f'{gg["direct"]["gal_h_mid"]:.3f} гал/год). {esc(G["_idle_note"])}</p>\n'
    ...
    # Вага вантажу проти вантажопідйомності — питання Івана 21.08 «наш трак потягне?».
    # Рендериться сюди ж, бо це та сама відповідь «чи повеземо», тільки з іншого боку.
    if "payload_check" in TRD:
        pc = TRD["payload_check"]
        how += ('<ul class="facts">\n' +
                "".join(f'<li>{esc(str(k))}: <b>{esc(str(v))}</b> фунтів</li>\n'
                        for k, v in pc["load_lbs"].items()) +
                f'<li>Вантажопідйомність машини: <b>{esc(pc["canyon_payload_lbs"])}</b></li>\n'
                f'<li><b>{esc(pc["verdict"])}</b></li>\n'
                f'<li>{esc(pc["how_to_confirm"])}</li>\n</ul>')
    rows = []
