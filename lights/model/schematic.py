...
swap logic) belongs to the power node and lives in solar/model/schematic.py.
  ...
  12V from station --> щит подіуму --> Гр.1 ШІМ-диммер -> 8x MR16
                                   ...
                                   --> Гр.3А врізні вогні торця + маркер ящика (аварійна)
"""
...
with schemdraw.Drawing(file=OUT + ".svg", show=False) as d:
    ...
    d += elm.Label().at((8.0, -10.6)).label(
        ...
        "прожектори і декор, а врізні вогні торця тримаються найдовше — щоб у темряві\n"
        "ніхто не наштовхнувся на подіум. Аварійна лінія за ніч з'їдає навіть трохи більше за декор.\n"
