...
Живлення і дані заходять У ЦЕНТРІ: один магістральний кабель від коробки
контролера йде всередину настилу до центру, і там паралельно розходиться на
вісім рукавів. Це рішення Івана — вести живлення до восьми зовнішніх кінців
по периметру означало б у рази більше міді. Напрямок біжучого фронту від
місця вводу не залежить: сегмент реверсується в прошивці.
...
def svg():
    ...
    # Вхід живлення і даних — У ЦЕНТРІ, біля основи кожного променя.
    # Рішення Івана: усі вісім рукавів зводяться в одну точку під настилом,
    # туди заходить один магістральний кабель — так міді треба в рази менше,
    # ніж вести живлення до восьми зовнішніх кінців по периметру.
    # Напрямок біжучого фронту від цього не залежить: він задається реверсом
    # сегмента у прошивці, а не тим, з якого кінця фізично прийшов дріт.
    for k in range(N):
        a0 = -pi / 2 + 2 * pi * k / N
        x = CX + R_RING_M * cos(a0) * scale
        y = CY + R_RING_M * sin(a0) * scale
        o.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="7" fill="none" '
                 f'stroke="#e08b3e" stroke-width="1.6"/>')

    # сама точка зведення: коробка під настилом у центрі подіуму
    o.append(f'<circle cx="{CX}" cy="{CY}" r="16" fill="none" stroke="#e08b3e" '
             f'stroke-width="1.4" stroke-dasharray="4,3"/>')
    o.append(f'<text x="{CX}" y="{CY + 4}" text-anchor="middle" font-size="9" '
             f'fill="#e08b3e">вузол</text>')
    ...
    o.append(f'<circle cx="34" cy="{H-66}" r="6" fill="none" stroke="#e08b3e" stroke-width="1.6"/>')
    o.append(f'<text x="48" y="{H-62}" font-size="10" fill="{txt2}">'
             f'вхід живлення і даних — у центрі, під настилом</text>')
    o.append(f'<text x="34" y="{H-44}" font-size="10" fill="{txt2}">'
             f'один магістральний кабель у центр, там паралельно на вісім рукавів'
             f'</text>')
    o.append(f'<text x="34" y="{H-20}" font-size="10" fill="{txt2}">'
